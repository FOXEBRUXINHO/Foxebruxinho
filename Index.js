	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hello @${num.split('@')[0]}\nBem vindo ao grupo não seja um ghost ❤*${mdata.subject}*`
				...
				teks = `Ghost fdp 🐊 @${num.split('@')[0]} ja foi tarde👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ Que bom que foi, ufa ✔️',
				error: {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado pelos adm do grupo! ❌',
					Badmin: '❌ Este comando só pode ser usado quando o bot é adm do grupo! ❌'
				}
			}
        ...
        case 'promote':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso o recurso de boas-vindas no grupo ✔️')
					} else if (Number(args[0]) === 0) {
